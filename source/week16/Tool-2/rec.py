...
class VoiceRec(QObject):
	...
	def record(self, path, name, Gui):
		...
		self.noisered(out)
		self.plot(out, Gui)
		if self.checkstartsilence(WAVE_OUTPUT_FILENAME) == False or self.checkendsilence(WAVE_OUTPUT_FILENAME) == False or self.checkfrequency(WAVE_OUTPUT_FILENAME) > 32000:
	...
	def getstartslice(self, inputtaken):
		""" This function takes the start slice from a input wav file"""
		output = inputtaken.split('.')[0] + 'start.wav'
		...
		time0, time1 = duration, duration1 # get audio for the start 100 miliseconds
	...
	def checkstartsilence(self, inputgiven):
		""" This function checks for the startsilence of an input wav file"""
		check = 0
		output = self.getstartslice(inputgiven)
